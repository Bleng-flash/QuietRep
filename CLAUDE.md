...
- **Known emulator-only "bug" (investigated, NOT a code bug):** bodyweight entry times once appeared 8 hours behind local (SGT) time during testing. Root cause was the **Android emulator's own clock — emulators default to GMT** (verified via `adb shell getprop persist.sys.timezone` → `GMT`), so every `toLocale*` render faithfully showed UTC. No app code was changed; the fix is the emulator's setting (Settings → System → Date & time → set the time zone, or launch with `emulator -timezone Asia/Singapore`). **This is a non-issue on a physical phone in a real release** — handsets carry the correct local timezone (network-provided), so the same stored UTC data renders in the user's local time. If times ever look shifted by a whole-hour offset app-wide, check the device clock first, not the code.
