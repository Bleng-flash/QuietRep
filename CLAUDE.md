...
- **Step 2 — Units preference (kg / lbs) — IN PROGRESS (commit 1 of 3 done).** Weights are **stored as typed**, in the unit selected at log time (the user's explicit choice — *not* canonical-kg-with-display-conversion). Pre-release with 0 users, so there is no migration and every new field is required. Naming is fixed and binding: the type is `WeightUnit`, the field on a stored entity and on the setting is `unit`, the prop meaning "the unit these numbers were logged in" is `loggedUnit`, and the local alias for the current setting inside a converting card is `displayUnit`. No other name for a unit appears anywhere. Remaining commits:
  - **Commit 2 — sessions record their unit** (pure plumbing, no visible change): `WorkoutSession.unit` and `ExercisePerformance.unit` (both required); `toCanonicalSession` gains `unit` in 4th position so every persist path writes it; `SessionBuffer` gains `unit`, with `ActiveSessionProvider` reading `useUnit()` and `startSession` stamping it **at creation**, not at finish — the buffer owns it for its whole life, and `finishActiveSession()` therefore stays param-less (the session's own recorded unit is authoritative, not whatever the setting happens to be when Finish is tapped). `collectExercisePerformances` copies `unit: session.unit` across, for the same reason it already copies `sessionName`. `MAX_WEIGHT` 1000 → 1500 with a unit-agnostic comment; `isLoggedSetValid` stays unit-agnostic.
