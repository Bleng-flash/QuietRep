...
The absent prune pass is a decision, not an oversight. Deleting a stored default that past sessions reference creates a dangling FK — tolerated, rendering "Unknown exercise" — so pruning trades an undeletable row for a corrupted history entry. That is a data-quality call and it has not been made; raise the tradeoff explicitly before proposing one. The display-order sort below places orphans sensibly (end of their muscle group's defaults block) but does **not** remove them — do not mistake that for a fix.
