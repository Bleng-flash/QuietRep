...
- **Step 2 — Units preference (kg / lbs) — TODO.** A global unit setting propagated to every load label/input, with weights **stored in the selected unit** (the user's explicit choice — *not* canonical-kg-with-display-conversion). Plan:
  - Types (`src/types/index.ts`): `type WeightUnits = 'kg' | 'lbs'`; add `unit: WeightUnits` to `WorkoutSession` (session-level — the global unit is stable within one session). Read-time default `'kg'` for pre-existing stored sessions that lack the field.
  - New `src/storage/settings.ts`, key `@quietrep/settings`, a single JSON object (the `getActiveSession`/`setActiveSession` JSON single-value pattern, not the bare-string `activeSplit` one): `getSettings()` → `{ units }` with defaults, `updateSettings(partial)`. Export from the `storage/index.ts` barrel.
  - New `src/context/SettingsContext.tsx` — same provider template as `ThemeContext`, a **separate** provider (units is a plain scalar, distinct from the theme machinery). Exposes `{ units, setUnits }`; mount in `_layout.tsx`. This is the reactive subscription layer that makes the label update everywhere with no prop-drilling.
  - Propagation: replace the 3 hardcoded `Load (kg)` headers with `Load (${units})` — `SessionExerciseCard.tsx` (live session, current setting) and the two read-only detail cards `ReadOnlySessionExerciseCard.tsx` + `ExercisePerformanceCard.tsx`, which render **past** data so must label with the **session's stored `unit`** (a set logged in kg stays "kg" after the user switches to lbs). Thread the session `unit` down to those cards.
  - Session tagging: `finishActiveSession` (`ActiveSessionContext`) stamps the current `units` onto the session at finish, beside `finishedAt`. Validation (`isLoggedSetValid`, `MAX_WEIGHT`) stays unit-agnostic numeric.
  - Profile: add a **Units** section — a `SegmentedControl` (kg / lbs) → `setUnits`.
