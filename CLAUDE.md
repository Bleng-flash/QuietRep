...
- **Step 3 — Bodyweight log + progression chart — IN PROGRESS.** `BodyweightEntry = { id, weight, unit: WeightUnit, recordedAt }` (unit tagged per entry — logged over time, setting may change between entries). New `src/storage/bodyweight.ts`, key `@quietrep/bodyweight`, array pattern: `getBodyweightEntries()`, `addBodyweightEntry(Omit<BodyweightEntry,'id'>)`, `deleteBodyweightEntry(id)`; export from the barrel. **Now-only logging — storage is stored newest-first** (see "Bodyweight is 'now'-only logging" in Key Design Decisions): `addBodyweightEntry` prepends and `getBodyweightEntries` returns raw (no sort), so on-disk order is display order. **Logging and viewing are split by surface** (action vs review, like the workout FAB vs History): **logging lives on Home** (`src/components/home/LogBodyweightCard.tsx` — a self-contained card between the "This month" stats and "Recent workouts"; owns its input buffer, reuses `sanitizeWeight`, disabled-until-valid guard, stamps `unit` from `useUnit()` at log time, writes `addBodyweightEntry` itself, clears + success-haptic feedback), and **viewing/managing lives on Profile** (`src/app/bodyweight.tsx` — a root drill-down screen reached from a Profile nav row, painting its own `layout.screen` background; a `FlatList` of `BodyweightEntryRow` showing weight + unit + `formatSessionDate(recordedAt)` **converted to the current display unit** like the History cards, delete via confirm `Alert`). **A progression chart is now in scope** (line graph over time) via a new `react-native-svg` dependency (user installs; `npx expo install react-native-svg`) and a hand-built `BodyweightChart` — the chart is a **later, paused stage** (the trend screen keeps the list until then; the delete affordance's final home is deferred to that stage). Plan file: `/home/bryan/.claude/plans/read-claude-md-to-understand-velvety-crown.md`.
...
- **Storage is stored newest-first, with no sort.** `addBodyweightEntry` prepends (`[entry, ...allEntries]`) and `getBodyweightEntries` returns the raw array. Because entries can only be created "now", insertion order *is* reverse-chronological order, so the on-disk order already equals the display order — read and write agree, and no sort-on-read is needed (the `sessions.ts` model, not the `workouts.ts` append-then-sort-elsewhere one). **If date back-dating/editing is ever added, this assumption breaks** — at that point either prepend must become an insertion-sorted write or `getBodyweightEntries` must sort on read, and `BodyweightChart` (which today just reverses this array to plot oldest→newest) must sort defensively.
...
Because logging is a present-moment *action* and viewing the trend is *review*, the two are **split by surface** — the same action-vs-review separation the app already makes for workouts (the FAB starts one; the History tab reviews it). **Logging lives on the Home tab** (`LogBodyweightCard`, the "today" surface); **viewing/managing lives on the Profile trend screen** (`src/app/bodyweight.tsx`, reached from a Profile nav row). This reinforces the now-only rule: you log on the surface that means "now". The Home card is self-contained (owns its buffer, reads the unit, writes the entry, gives its own save feedback) because Home renders no bodyweight state and there is one fixed action — the over-prop / "own behaviour at the right level" rule, like `RecentWorkouts` owning its own navigation.
...
- `src/components/` is split into per-tab subdirectories:
  ...
  - `profile/` — components owned by the Profile tab's bodyweight trend screen: BodyweightEntryRow (and, later, BodyweightChart)
