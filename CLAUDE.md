...
1. **Dogfooding on the preview build — DONE and ongoing.** `eas build --profile preview --platform android` was built and installed on a physical Android phone in Aug 2026; day-to-day iteration since is `eas update --branch preview` (see Build notes below). Unlike `development`, `preview` embeds the JS bundle, so it runs with no Metro server and can be handed to other Android testers via the EAS internal-distribution link. (`production` is the wrong profile: it emits an `.aab` for the Play Store, which cannot be sideloaded.) **All three bugs from the first dogfooding pass are closed** — two fixed, one (the centred-`TextInput` scroll steal) closed as won't-fix with the cause documented. Keep using the app in real gym sessions; this is what surfaces the remaining rough edges, and it has paid for itself three times.
2. **README polish** — `README.md` is already substantially written user-facing prose; what remains is screenshots (there is a placeholder comment) and a final pass.
3. **Backend transition.** Carries the deferred items with it:
...
**`eas update` semantics — each of the first three has caught us out:**
- It bundles the **working directory, not a commit.** A dirty tree ships whatever is on disk, and the recorded `gitCommitHash` then describes something other than what shipped (check `isGitWorkingTreeDirty` via `eas update:view <GROUP_ID> --json`). Corollary: reverting a file is enough to un-ship it — no commit needed first.
- It takes **two launches to apply** — one downloads, the next runs it. A "the fix didn't work" report immediately after publishing is usually just the old bundle.
- `runtimeVersion` uses the **fingerprint** policy, so a native-dependency change makes the update publish successfully and the phone **silently never receive it**. Verify what a channel actually points at with `eas channel:view preview`.
- Prefer a check the *device* can answer over a published-side one: a visible in-app change proves the bundle is running, whereas a commit hash only proves what was uploaded.

Wiping device data: **Settings -> Apps -> QuietRep -> Storage -> Clear storage, without uninstalling.** Android Auto Backup (`allowBackup` defaults true) restores app data on reinstall, so uninstall/reinstall does *not* clear it. Note `development` and `preview` build the same package (`com.bleng.quietrep`) with no variant suffix, so they share one data sandbox.
...
**Adding a name to `DEFAULT_EXERCISES` is a ONE-WAY DOOR — be conservative about what goes in.** `seedDefaultExercises` only ever appends; there is no prune pass. A default **removed** from the constant in a later release stays in `@quietrep/defaultExercises` on every device that already seeded it, and since `deleteExercise` only touches the user key, the user cannot remove it either. It is undeletable, permanently, with no in-app remedy — clearing app data is the only cure, and that costs the user their entire history. Not hypothetical: the phone that ran both the dev and preview builds (same package, so one sandbox) accumulated exercises from constants that no longer existed and had to be wiped in Aug 2026. **Treat every default as permanent; leave a marginal exercise out rather than add one you might reconsider.**

The absent prune pass is a decision, not an oversight. Deleting a stored default that past sessions reference creates a dangling FK — tolerated, rendering "Unknown exercise" — so pruning trades an undeletable row for a corrupted history entry. **That call has been made (2026-08-08): never prune.** A user who logged a since-removed default must keep both the ability to keep logging it and the integrity of its past history, so an undeletable row is the accepted cost and a dangling FK is not. `seedDefaultExercises` stays append-only permanently — **do not propose a prune pass, and do not re-raise the tradeoff.** This is what makes the one-way-door warning above binding rather than advisory: nothing downstream will ever clean up a careless addition. The display-order sort below places orphans sensibly (end of their muscle group's defaults block) but does **not** remove them — do not mistake that for a fix.

**Exercise display order is a read-time sort in `getAllExercises`, and storage order is deliberately NOT normalised.** Neither key stores exercises in a displayable order: `seedDefaultExercises` **appends** newly-added defaults to the tail of the stored array, so a device that seeded in two passes holds two separated chunks of the same muscle group and drifts further from `DEFAULT_EXERCISES` with every release; user exercises live in a second key entirely, so they sort after every default regardless of group. `getAllExercises` delegates to `orderExercisesForDisplay` (`utils/exercise.ts`), ranking by muscle group (`MUSCLE_GROUPS` index) → default-before-user → position within origin (`DEFAULT_EXERCISES` index for defaults; array index for user exercises, which **is** creation order because the user key is append-only). A stored default the constant no longer lists sinks to the end of its group's defaults block, keeping its stored relative position.

This is the `ORDER BY` of a future `GET /exercises` and belongs at that seam, not in a screen. Two consequences: `PickerModal` documents `items` as *already in display order* and trusts its caller, so the **storage seam is the only place order can be changed** for the flat pickers; and `ExerciseListScreen`'s `SectionList` grouping still owns the headers but no longer owns ordering — it agrees with the util only because both key off `MUSCLE_GROUPS`, so the two must move together. Because no order is persisted, changing the ranking needs no migration. Contrast bodyweight, where insertion order *is* the display order and there is no sort at all.

**There is exactly ONE public list read, and an unordered second one must not be added.** Splitting into `getAllExercises` (unordered) + `getAllExercisesForDisplay` was considered and rejected. Of its eight call sites, four need order — and two of those (`WorkoutEditor`, `WorkoutSession`) feed **one** fetch to both an `exerciseMap` and an `ExercisePicker`, so "builds a map" does not identify a caller that can skip ordering. More decisively, `GET /exercises` is one endpoint with one deterministic `ORDER BY`; an unordered public variant would be a storage function with nothing behind it at backend time, and choosing the shorter, more obvious name for a future picker would silently reproduce the jumbled list this sort exists to fix — no type error, no crash. If a second order is ever genuinely wanted, add a **parameter** mirroring `?sort=` (as `getExercisePerformances` does for `?limit=`), not a second function.

Callers *inside* `exercises.ts` that only find or count use the module-private `readAllExercises()` — the plain concat, unexported so no screen can reach it. `getExerciseById` uses it; **`addExercise` deliberately does not**, because it needs `userExercises` as a separate array to write back and a merged one would force the redundant third read its own comment warns about.
...
**Every single-line `TextInput` spreads one slack token — `INPUT_SLACK` or `COMPACT_INPUT_SLACK` from `spacing.ts`. Never re-type its three properties by hand.** The token bundles `paddingVertical: 0`, a `minHeight` floor, and `textAlignVertical: 'center'`; a field must never size itself from padding alone. RN measures a `TextInput`'s box with `Typeface.DEFAULT` (`TextLayoutManager.kt:619-625`) while the native `EditText` draws with the **theme's** typeface (`ReactEditText.kt:597`); on a device where those differ — a vendor font picker (Xiaomi/Samsung), a CJK UI font, the Bold-text accessibility adjustment — the drawn line is taller than the measured box, the widget scrolls it up to keep the caret visible, and the glyph tops are **clipped**. Every input shipped with zero slack, so a tester's Redmi clipped all 12 of them while the developer's phone and the emulator showed nothing. Full narrative in [docs/android-textinput-font-metrics.md](docs/android-textinput-font-metrics.md).

- Two tiers, both multiplied by a clamped `PixelRatio.getFontScale()` so the slack stays proportional as text scales: **`INPUT_SLACK`** (floor 40) for standard bordered fields, **`COMPACT_INPUT_SLACK`** (floor 32) for the dense set rows and the session title. A new input picks one. That scale is a launch-time snapshot and the clamp is load-bearing — `getFontScale()` falls back to the pixel *density* if the native value is missing, which would make a 40dp field 110dp.
- **Why a token and not three properties: a field carrying two of the three looks correct on a device whose UI font matches `Typeface.DEFAULT`.** Three fields shipped exactly that way (`picker.searchInput`, `WorkoutEditor`/`SessionHeader` name inputs — floor and alignment, no padding reset) and were found only by audit. Bundling makes a partial application unwritable. The bare `INPUT_MIN_HEIGHT`/`COMPACT_INPUT_MIN_HEIGHT` constants are **module-private inside `spacing.ts`** — not exported at all, so the floor cannot be taken without the rest. Same reasoning as `readAllExercises()` in `exercises.ts`: an incomplete primitive stays unexported so no caller can reach for it by mistake.
- **`paddingVertical: 0` is the part that looks redundant and is not.** Under the New Architecture (`newArchEnabled`), `AndroidTextInputComponentDescriptor` reads the **theme's** default TextInput padding and applies it as the component's default, so a field with no padding style keeps it and the usable interior becomes `minHeight - themePadding`. Zeroing it is what converts padding into interior room.
- The tokens use **`satisfies TextStyle`, not `: TextStyle`** — an annotation widens them, and `layout.inputField` (which spreads `INPUT_SLACK`) is also applied to a `Pressable` in `exercise/new.tsx`, a `ViewStyle` slot. `satisfies` type-checks without widening. Horizontal padding is deliberately outside the token: it varies per field and has nothing to do with vertical clipping.
...
That container is also the sheet's `KeyboardAvoidingView` (see "A bottom sheet is the same rule" below), which has one consequence worth knowing before you go looking for it: **an absolutely-positioned child lays out against its parent's *padding* box**, so while the keyboard is up the backdrop stops at the IME's top edge rather than running to the window bottom. It is masked, since the keyboard is opaque and occupies exactly that strip. Do not "fix" it by hoisting the backdrop out of the avoider — that costs the sibling structure the whole rule above depends on.
...
The convention: **exactly one `<KeyboardProvider>` exists, at the root in `_layout.tsx`** (see the Modal rule below — this is a prohibition, not a description); every avoiding surface imports `KeyboardAvoidingView` from **`react-native-keyboard-controller`** (not `react-native`) with **`behavior="padding"` on both platforms** — the `Platform` gate is gone, it was the literal reason Android did nothing. RN's own component uses the *identical* geometry formula; what the swap replaced is the measurement source, since RN derives the keyboard's top edge from `getWindowVisibleDisplayFrame().bottom`, a frame that does not shrink for the IME under edge-to-edge.
...
- **That exemption covers the *input*, not the surface.** If a **scrollable list sits below** the top-pinned search bar, the surface still needs a KAV — for the list, not the field. The list's frame runs to the bottom of the window, under the IME, and a scroll can only travel `contentHeight - frameHeight`, so its tail and any footer can never be pulled above the keyboard: unreachable exactly while the user is typing to filter. `PickerModal` shipped this way and is the case that established the rule. The tell is that a *fixed* `paddingBottom` looks like it should cover it and cannot — the padding has to come from the keyboard's real height. **`ExerciseListScreen` and `WorkoutFabMenu` were the last two surfaces still missing one; both are fixed and the sweep is complete.** Every `TextInput` in the app now either sits under a surface that owns a KAV or is a child of one (`LogBodyweightCard` under `(tabs)/index`, `PlannedSetRow` under `WorkoutEditor`, `LoggedSetRow`/`SessionHeader` under `WorkoutSession`) — so a *new* input needs a KAV audit, but the existing ones do not.
- **A bottom sheet is the same rule with a worse failure and a different mechanism.** When the surface is docked to the window bottom (`container` `flex: 1, justifyContent: 'flex-end'`), the IME does not merely cut off a list's tail — it covers the **whole sheet, the input included**, so the user cannot see what they are typing. The fix is still one `behavior="padding"` KAV, but it works by a different route: the padding shrinks the *container's* content box and flex-end re-docks the sheet to the new bottom edge, moving it up as a unit. Two things then fall out for free, and both must stay: a percentage `maxHeight` on the sheet re-resolves against the reduced content box, so the sheet caps itself proportionally instead of overflowing upward; and the scroll area's `flexShrink: 1` absorbs the difference, so the list shortens rather than the sheet clipping. `WorkoutFabMenu` is the case.
- **RN `<Modal>` rule (binding): NEVER nest a second `<KeyboardProvider>`, inside a Modal or anywhere else.** A KAV inside a Modal is served by the root provider and needs nothing added. A second provider does not merely duplicate work — it **kills keyboard tracking app-wide**: each provider registers its own `ModalAttachedWatcher` on the *global* event dispatcher, both react to the same modal show, and both call `setOnDismissListener`, which is a **setter**, so the second overwrites the first. Only one resume ever fires and the loser's callback stays suspended for the rest of the process (`isSuspended` is a plain boolean, not a refcount). This shipped, and cost a wrong fix and a revert before it was found. Full narrative in [docs/android-keyboard-modal-provider.md](docs/android-keyboard-modal-provider.md). A *centered* dialog still needs a `keyboardVerticalOffset` so it clears the keyboard with a visible gap; a **bottom sheet takes none** — it should sit flush against the IME, which is what flex-end already gives it.

**Three different things cross a `<Modal>` boundary differently — conflating them is what produced the bug above.** A Modal is a separate native window whose children are still React descendants, so:

| Crosses the boundary? | What | Consequence |
| --- | --- | --- |
| **No** | Safe-area insets | The Modal owns its own `useSafeAreaInsets()` — see "A full-screen `Modal` is a chrome-bearing surface" above |
| **Yes** | React context, and the responder walk | The root `KeyboardProvider` reaches modal content; a host screen's scrollable governs taps inside the modal |
| **Bridged** | Native IME measurement | Does not cross by itself, but `ModalAttachedWatcher` attaches to the Dialog's own window and dispatches through the **root** provider (`eventPropagationView`) — so it reaches you anyway, and adding your own provider is what breaks it |

Reasoning from the first row to the third is the exact mistake: "insets don't cross" is true and says nothing about context or about what the library already does for you.
...
Fix, in one place: `panGesture={Gesture.Pan().activateAfterLongPress(PAN_ACTIVATE_AFTER_LONG_PRESS_MS)}`, that constant being 220 in `DraggableCardList`. A swipe now moves long before 220 ms of stillness, so the pan never arms and the outer scroller wins uncontested from anywhere on a card. All three consumers inherit it. **The binding rule: pan activation must be >= the drag `Pressable`'s `delayLongPress`.** All drag surfaces use the exported `DRAG_LONG_PRESS_DELAY_MS` (200), and the 20 ms margin means the drag arms just before the pan. Any future drag surface must keep its `delayLongPress` <= 220.

**Takeaway:** this app has **two** known causes of a swallowed scroll, and they need different fixes — check both. (1) An **eager pan with no activation threshold** competing with an outer scroll, the case above; the RNGH remedies are `activateAfterLongPress` / `activeOffset` / `hitSlop`. (2) A **centred `TextInput`**, per the known-bug entry below, which is native RN behaviour and has no JS remedy. The tell separating them: the `TextInput` case only fails when the gesture starts *on a field*, and leaves a vanished placeholder behind.

**A centred `TextInput` steals scroll gestures on Android — KNOWN BUG, deliberately NOT fixed.** Every `TextInput` blocks its parent ScrollView on touch-down unconditionally and releases it on the first move *only if it cannot scroll itself* (`ReactEditText.kt:295-316`). `textAlign: 'center'` maps to `Gravity.CENTER_HORIZONTAL` (`ReactTextInputManager.kt:567`), and a centred single-line `EditText` reports a non-zero horizontal scroll range where a left-aligned one reports none — so it never releases, swallowing the scroll and sliding its own placeholder out of view. Deterministic repro: type a character, delete it, swipe across the field. All four centred fields in the app fail it; all three left-aligned ones pass.

The **only complete fix is dropping `textAlign: 'center'`**, rejected because centred set-row values are a deliberate visual choice and the practical cost is an occasional failed scroll. **Do not "fix" this by rendering our own placeholder** — that preserves centring but is cosmetic only, leaves the scroll theft untouched, and deletes the one visible signal that it is happening. Full narrative, evidence table and the two candidate fixes in [docs/android-textinput-centered-scroll-steal.md](docs/android-textinput-centered-scroll-steal.md). **Adding `textAlign: 'center'` to any new `TextInput` inside a scrollable opts into this**; a field in pinned chrome (`SessionHeader`) has no touch to steal and is unaffected.
...
- **When the provocation is an unreliable gesture, a non-reproduction is not evidence of absence — and a matrix built from such negatives is fiction.** Find a deterministic trigger *first*; treat every discriminator recorded before one exists as unverified. Bug 2 accumulated five confident "findings" (breaks in the session but not the editor, needs a template-seeded session, needs `targetMinReps`, only on first open, persists only via one start path) and **every one was an artefact** of one flaky swipe succeeding at different rates. A deterministic repro found by accident collapsed all five in a single step. See [docs/android-textinput-centered-scroll-steal.md](docs/android-textinput-centered-scroll-steal.md).
- **A fix that makes the symptom worse — especially wider — means the diagnosis was wrong, not that the fix was incomplete.** Distinct from the line above: a fix that *changes* a symptom points to a second cause, but one that spreads a local failure app-wide means you built on an inferred mechanism instead of a verified one. Revert first, then verify before touching anything. The `KeyboardSpacer` rewrite took "some screens don't avoid" to "no screen avoids" and that was the signal.
- **Before writing a workaround for a platform primitive, check whether the library already supports it.** `react-native-keyboard-controller` ships explicit RN `<Modal>` support; the hand-rolled nested-provider workaround *was* the bug. Grep the library's source for the primitive's name before assuming it is unhandled.
- **Two lines of our own docs disagreeing about a mechanism is a defect, not a nuance.** CLAUDE.md asserted both that the responder walk crosses `<Modal>` boundaries and that the keyboard context does not, nineteen lines apart; the edge-to-edge doc did the same seven lines apart. The wrong version won both times because it was the one attached to an instruction. Reconcile on sight.
...
- Never use en/em dash literals **inside data strings** — anything that renders as content or gets persisted. Use a plain hyphen (e.g. rep ranges: `${min}-${max}`, never `${min}–${max}`). The reason is confusability: VSCode's `unicodeHighlight` flags them as ambiguous with the ASCII hyphen-minus, and a dash sitting between digits is genuinely easy to misread as one.
- **One sanctioned exception: a standalone dash used as a placeholder glyph** — `PlannedSetRow`'s `placeholder="—"`. There the character *is* the entire string, doing an icon's job rather than standing in for a hyphen, so nothing is confusable. Deliberate and decided; **do not "correct" it to a plain hyphen.**
