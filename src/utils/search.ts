// Returns true if every word in the search query partially matches at least one word in the item's name.
// e.g. "inc ben" matches "Incline Bench Press" because "inc" matches "Incline" and "ben" matches "Bench".
export function matchesSearchQuery<ItemType extends { name: string }>(
  item: ItemType,
  searchQuery: string,
): boolean {
  const queryWords = searchQuery.toLowerCase().trim().split(/\s+/).filter(Boolean);
  if (queryWords.length === 0) return true; // empty queries returns true => match all items

  const nameWords: string[] = item.name.toLowerCase().split(/\s+/);
